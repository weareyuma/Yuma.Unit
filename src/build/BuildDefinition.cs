...
file class BuildDefinition : NukeBuild
{
	...
	Target AzureFeedSetup => td => td.Unlisted()
		.Description("Set PushApiUrl/PushApiKey for Azure Artifacts when on feature branch.")
		.OnlyWhenDynamic(() => IsFeatureBranch)
		.Requires(() => AzureFeedApiKey)
		.Requires(() => AzureFeedApiUrl)
		.Executes(() => {
			PushApiKey = "AzureDevOps"; // Azure ignores API key
			PushApiUrl = AzureFeedApiUrl;
		});
		...
		.Executes(() => {
			...
			NuGetPackagesDirectory.GlobFiles("*.nupkg")
				.ForEach(path => {
					...
					DotNetNuGetPush(s => s.EnableSkipDuplicate()
	...
	[Parameter("Azure Artifacts feed API key, i.e. a PAT with packaging scope.")]
	[Secret]
	readonly string AzureFeedApiKey;

	[Parameter("Azure Feed API URL, e.g. https://pkgs.dev.azure.com/<org>/_packaging/<feed>/nuget/v3/index.json.")]
	readonly string AzureFeedApiUrl;
